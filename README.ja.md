...
音源チップは自分のレート（1 秒あたりのサンプル数）で動きます：OPL3 は 49716，
OPN2 は 53267，OPNA は 55466 です．ホスト（DAW など）が求めるのは 44100 や
48000 といった自分のレートなので，どこかで変換しなければなりません．

2 つのレートに公約数があるとき，ADLplug-Next はチップ自身のサンプルを受け取り，
そのレートの組のために作ったフィルタで変換します．高い方の 1 オクターブが失われる
ことも，聞こえる帯域に折り返って混ざることもありません．OPL3 はホストが求める
レートのどれでもこちらになります．

公約数がないときは，その形のフィルタには数百万個の数値が必要になります（53267 は
素数なので，どのホストのレートとも公約数がありません）．そのときは変換を
libADLMIDI・libOPNMIDI の中の直線補間に任せます——以前と同じです．いまのところ
OPN2 と OPNA はこちらになります．

変換の設定は，エミュレータのメニューの「Resampling...」にあります．どこで変換する
か（プラグイン自身のフィルタか，以前どおりライブラリの直線補間か）と，フィルタの
作り方を選べます．名前のついた設定——fast，good（既定），best，extreme——は出発点
にすぎず，その裏にある数値はすべて手で変えられます（阻止域をどこまで下げるかから，
フィルタが持ってよい係数の数まで）．プラグインはどれにも上限を設けません：作れない
フィルタはその理由を示し，代わりにライブラリの補間が鳴ります．「Max taps」を上げれ
ば OPN2 と OPNA にもフィルタが作れますが，メモリ（数十 MB）を使います——どれだけ
使うかは，適用したあとにウィンドウに表示されます．

この設定はプロジェクトと一緒に保存されます．自動化できるパラメータではありません：
変えるたびにフィルタを作り直すので，少し時間がかかるからです．
