...
struct Instrument : OPN2_Instrument
{
    ...
    [[nodiscard]] WOPNInstrument to_wopl() const noexcept;
...
#undef OP_PARAMETER
    ...
    [[nodiscard]] bool equal_instrument(const OPN2_Instrument &o) const noexcept;
    [[nodiscard]] bool equal_instrument_except_delays(const OPN2_Instrument &o) const noexcept;
};
...
struct Bank_Id : OPN2_BankId
{
    constexpr Bank_Id() noexcept
        : OPN2_BankId{.percussive = 0, .msb = 0xff, .lsb = 0xff} {}
    constexpr Bank_Id(std::uint8_t bank_msb, std::uint8_t bank_lsb, bool is_percussive) noexcept
        : OPN2_BankId{.percussive = static_cast<OPN2_UInt8>(is_percussive), .msb = bank_msb, .lsb = bank_lsb} {}
